        ...
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px', flexWrap: 'wrap', gap: '20px' }}>
                ...
                <button
                    ...
                    style={{ background: showForm ? 'var(--color-accent)' : '#6d8243' }}
                >
            ...
            {showForm && (
                <div className="glass-panel" style={{ background: 'rgba(255, 253, 250, 0.5)', color: 'var(--zoo-muted)', padding: '20px', marginBottom: '30px', border: '1px solid var(--color-primary)' }}>
                    ...
                    <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                        ...
                        <div style={{ gridColumn: '1 / -1', marginTop: '10px' }}>
                            <button type="submit" className="glass-button" style={{ background: '#6d8243', width: '100%' }}>Save Animal</button>
                        </div>
                    </form>
                </div>
            )}
            ...
            {loading ? (
                ...
                <div className="grid-cards">
                    {filteredAnimals.map(animal => (
                        <div key={animal.animal_id} className="glass-panel" style={{ background: 'rgba(255,255,255,0.5)', padding: '20px', position: 'relative' }}>
                            ...
                            <button
                                className="glass-button"
                                style={{ background: '#6d8243', width: '100%', fontSize: '12px', padding: '8px' }}
                                onClick={() => {
            ...
            {selectedAnimal && (
                <div style={{
                    ...
                    <div className="glass-panel" style={{ width: '600px', maxWidth: '90vw', maxHeight: '80vh', overflowY: 'auto', padding: '30px', background: 'rgb(255, 253, 250)', border: '1px solid var(--glass-border)' }}>
                        <div style={{ color: 'var(--zoo-muted)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                <Activity color="var(--color-accent)" />
                                <h2 style={{ margin: 0 }}>Medical History: {selectedAnimal.name}</h2>
                            </div>
                            <button onClick={() => setSelectedAnimal(null)} style={{ background: 'none', border: 'none', color: 'var(--zoo-muted)', cursor: 'pointer' }}><X /></button>
                        </div>
                        ...
                        {!showHistoryForm ? (
                            <button
                                ...
                                style={{ width: '100%', marginBottom: '20px', background: '#6d8243' }}>
                                + Add Medical Entry
                            </button>
                        ) : (
                            <form onSubmit={handleHistorySubmit} style={{ marginBottom: '30px', background: 'rgba(255,255,255,0.05)', padding: '20px', borderRadius: '10px' }}>
                                <h4 style={{ color: 'var(--zoo-accent)', marginTop: 0 }}>New Entry</h4>
                                ...
                                <div style={{ display: 'flex', gap: '10px', marginTop: '15px' }}>
                                    <button type="submit" className="glass-button" style={{ background: 'var(--zoo-accent)', flex: 1 }}>Save</button>
                                    <button type="button" className="glass-button" onClick={() => setShowHistoryForm(false)} style={{ background:'#ef4444', flex: 1 }}>Cancel</button>
                                </div>
                            </form>
                        )}
                                    ...
                                    medicalHistory.map(record => (
                                        <div key={record.history_id} style={{ background: 'rgba(255,255,255,0.05)', padding: '15px', borderRadius: '10px' }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                                                <span style={{ color: 'var(--zoo-accent)', fontWeight: 'bold' }}>{new Date(record.date_treated).toLocaleDateString()}</span>
                                                <span style={{ fontSize: '14px', color: 'var(--zoo-muted)' }}>Age: {record.animal_age_at_treatment}</span>
                                            </div>
                                            <p style={{ margin: '5px 0', color: 'var(--zoo-accent)' }}>{record.disease ? `Disease: ${record.disease} ` : ''}</p>
                                            <p style={{ margin: '5px 0', color: 'var(--zoo-muted)' }}>{record.injury ? `Injury: ${record.injury} ` : ''}</p>
                                        </div>
                                    ))
                                }
                            </div>
